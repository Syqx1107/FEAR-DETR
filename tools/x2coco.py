...
def voc_get_label_anno(ann_dir_path, ann_ids_path, labels_path):
    '''1.从labels文件中提取所有类别名并赋予id（从1开始）
       2.从ann_ids文件中提取annotions文件名(这里是xml文件)
       3.根据ann_ids文件中的文件名，拼接出annotions文件的路径
    返回：返回字典{类别名: 类别id, ……} 和 annotions文件路径列表
    '''
...
def main():
    parser = argparse.ArgumentParser(
        formatter_class=argparse.ArgumentDefaultsHelpFormatter) #指定了帮助信息的显示格式
    parser.add_argument(
        '--dataset_type',
    ...
    #args中的参数：(required=True是必传参数)dataset_type数据集; json_input_dir数据集的json文件路径; image_input_dir数据集的图片路径; output_dir输出的json文件路径; train_proportion训练集比例; val_proportion验证集比例; test_proportion测试集比例; voc_anno_dirvoc数据集的标注文件路径; voc_anno_listvoc数据集的标注文件列表路径; voc_label_listvoc数据集的类别列表路径; voc_out_namevoc数据集的输出文件名; widerface_root_dirwiderface数据集
    try:
        assert args.dataset_type in ['voc', 'labelme', 'cityscape', 'widerface'] #加上你的数据集类型
    ...
    if args.dataset_type == 'voc':
        assert args.voc_anno_dir and args.voc_anno_list and args.voc_label_list #保证这3个参数存在
