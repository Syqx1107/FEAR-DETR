...
@register_op #注册到全局的操作符列表中
class DecodeX(BaseOperator): #所有图像变换都继承基类BaseOperator
    def __init__(self):
        """ Transform the X-images data to numpy format following the rgb format

                """
        super(DecodeX, self).__init__()
    ...
    def apply(self, sample, context=None):
        ...
        if 'image' not in sample:
            ...
            #输入的是ir模态图像
            if 'data_ir' in sample['im_file']: #sample['im_file']图像文件的路径
                ir_im_file = sample['im_file'] #ir图像文件路径
                rgb_im_file = sample['im_file'].replace('data_ir', 'data_rgb') #配对的rgb图像文件路径
                with open(ir_im_file, 'rb') as f:
                    sample['image'] = f.read() #sample['image']是ir图像
                with open(rgb_im_file, 'rb') as f:
                    sample['image_1'] = f.read() ##sample['image']是rgb图像
                sample.pop('im_file')
            #输入的是rgb模态图像
            else:
        ...
        try:
            ...
            im_1 = sample['image_1']
            data = np.frombuffer(im, dtype='uint8')
            data_1 = np.frombuffer(im_1, dtype='uint8')
            im = cv2.imdecode(data, 1)
            im_1 = cv2.imdecode(data_1, 1)
            if 'keep_ori_im' in sample and sample['keep_ori_im']:
                sample['ori_image'] = im
                sample['ori_image_1'] = im_1
...
@register_op
class DecodeXtrain1(BaseOperator):
    def __init__(self):
        """ Transform the X-images data to numpy format following the rgb format
        从单个输入图像创建两个相同的输出作为两个模态的输入
                """
        super(DecodeXtrain1, self).__init__()

    def apply(self, sample, context=None):
        """ load image if 'im_file' field is not empty but 'image' is"""
        if 'image' not in sample:
            # Process the corresponding RGB image
            im_file = sample['im_file'] #图像文件路径

            with open(im_file, 'rb') as f:
                #读取图像并复制一份：sample['image']和sample['image_1']，分别用于模型的两个输入流
                sample['image'] = f.read()
                sample['image_1'] = sample['image']
            sample.pop('im_file') #删除im_file字段

           
        #将图像解码为numpy数组并从BGR转换为RGB颜色空间
        try:
            im = sample['image']
            im_1 = sample['image_1']
            data = np.frombuffer(im, dtype='uint8')
...
@register_op
class DecodeXtest(BaseOperator):
    def __init__(self):
        """ Transform the X-images data to numpy format following the rgb format
                """
        super(DecodeXtest, self).__init__()
...
@register_op
class DecodeXv1align(BaseOperator):
    def __init__(self):
        """ Transform the X-images data to numpy format following the rgb format
                """
        super(DecodeXv1align, self).__init__()
    ...
    def apply(self, sample, context=None):
        ...
        if 'image' not in sample:
            ...
            ir_im_file = sample['im_file']
            rgb_im_file = sample['im_file'].replace('data_ir', 'data_rgb')
            rgb_im_file = rgb_im_file.replace('jpeg', 'jpg')
            rgb_im_file = rgb_im_file.replace('PreviewData', 'RGB')

            with open(ir_im_file, 'rb') as f:
                sample['image'] = f.read()
            if  os.path.exists(rgb_im_file):
                with open(rgb_im_file, 'rb') as f:
                    sample['image_1'] = f.read()
            else:
                sample['image_1'] = sample['image']
...
@register_op
class DecodeXv1aligntest(BaseOperator):
    def __init__(self):
        """ Transform the X-images data to numpy format following the rgb format
                """
        super(DecodeXv1aligntest, self).__init__()
...
@register_op
class PermuteX(BaseOperator):
    def __init__(self):
        """
        Change the channel to be (C, H, W)
        """
        super(PermuteX, self).__init__()
...
@register_op
class PermuteXcat(BaseOperator):
    def __init__(self):
        """
        Change the channel to be (C, H, W)
        """
        super(PermuteXcat, self).__init__()
    ...
    def apply(self, sample, context=None):
        ...
        sample['image'] = im
        sample['image_1'] = im_1
        sample['image'] = np.concatenate((sample['image'], sample['image_1']), axis=0)
        if 'pre_image' in sample:
            pre_im = sample['pre_image']
            pre_im = pre_im.transpose((2, 0, 1))
            sample['pre_image'] = pre_im
...
class NormalizeImageX(BaseOperator):
    def __init__(self,
                 mean=[0.485, 0.456, 0.406],
                 std=[0.229, 0.224, 0.225],
                 is_scale=True,
        ...
        Args:
            ...
            norm_type (str): type in ['mean_std', 'none']
        """
        super(NormalizeImageX, self).__init__()
        self.mean = mean
        self.std = std
        self.is_scale = is_scale
        self.norm_type = norm_type
...
@register_op
class RandomDistortX(BaseOperator):
    """Random color distortion.
    Args:
        hue (list): hue settings. in [lower, upper, probability] format.
        saturation (list): saturation settings. in [lower, upper, probability] format.
    ...
    def __init__(self,
                 ...
                 random_channel=False,
                 prob=1.0):
        super(RandomDistortX, self).__init__()
        self.hue = hue
        self.saturation = saturation
        self.contrast = contrast
        self.brightness = brightness
...
@register_op
class RandomFlipX(BaseOperator):
    def __init__(self, prob=0.5):
        """
        Args:
            prob (float): the probability of flipping image
        """
        super(RandomFlipX, self).__init__()
        self.prob = prob
        if not (isinstance(self.prob, float)):
            raise TypeError("{}: input type is invalid.".format(self))
...
class ResizeX(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if not isinstance(im, np.ndarray):
            raise TypeError("{}: image type is not numpy.".format(self))
        if not isinstance(im_1, np.ndarray):
            raise TypeError("{}: image_1 type is not numpy.".format(self))

        # apply image and image_1
        if len(im.shape) == 3:
            im_shape = im.shape
        else:
            im_shape = im[0].shape
        ...
        if len(im_1.shape) == 3:
            im_1_shape = im_1.shape
        else:
            im_1_shape = im_1[0].shape
        ...
        if len(im_1.shape) == 3:
            im_1 = self.apply_image(sample['image_1'], [im_scale_x, im_scale_y])
            sample['image_1'] = im_1.astype(np.float32)
        else:
            resized_images = []
            for one_im_1 in im_1:
                applied_im_1 = self.apply_image(one_im_1, [im_scale_x, im_scale_y])
                resized_images.append(applied_im_1)

            sample['image_1'] = np.array(resized_images)
...
@register_op
class RandomExpandX(BaseOperator):
    """Random expand the canvas.
    Args:
        ratio (float): maximum expansion ratio.
        prob (float): probability to expand.
    ...
    def __init__(self, ratio=4., prob=0.5, fill_value=(127.5, 127.5, 127.5)):
        super(RandomExpandX, self).__init__()
        assert ratio > 1.01, "expand ratio must be larger than 1.01"
        self.ratio = ratio
        self.prob = prob
        assert isinstance(fill_value, (Number, Sequence)), \
    ...
    def random_crop(self, sample, fake_bboxes=False):
        ...
        for thresh in thresholds:
            ...
            if found:
                ...
                sample['image'] = self._crop_image(sample['image'], crop_box)
                sample['image_1'] = self._crop_image(sample['image_1'], crop_box)
                if fake_bboxes == True:
                    return sample
...
@register_op
class NormalizeBoxX(BaseOperator):
    """Transform the bounding box's coornidates to [0,1]."""

    def __init__(self):
        super(NormalizeBoxX, self).__init__()
...
@register_op
class BboxXYXY2XYWHX(BaseOperator):
    """
    Convert bbox XYXY format to XYWH format.
    """

    def __init__(self):
        super(BboxXYXY2XYWHX, self).__init__()
...
class PadX(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        sample['image'] = self.apply_image(im, offsets, im_size, size)
        sample['image_1'] = self.apply_image(im_1, offsets, im_size, size)
        if self.pad_mode == 0:
            return sample
        if 'gt_bbox' in sample and len(sample['gt_bbox']) > 0:
            sample['gt_bbox'] = self.apply_bbox(sample['gt_bbox'], offsets)
