...
import math
import numpy as np
from .operators import register_op, BaseOperator, Resize, ResizeX
...
@register_op
class BatchRandomResizeX(BaseOperator):
    """
    Resize image to target size randomly. random target_size and interpolation method
    Args:
        target_size (int, list, tuple): image target size, if random size is True, must be list or tuple
    ...
    def __init__(self,
                 ...
                 random_size=True,
                 random_interp=False):
        super(BatchRandomResizeX, self).__init__()
        self.keep_ratio = keep_ratio
        self.interps = [
            cv2.INTER_NEAREST,
            cv2.INTER_LINEAR,
    ...
    def __call__(self, samples, context=None):
        ...
        resizer = ResizeX(target_size, keep_ratio=self.keep_ratio, interp=interp)
        return resizer(samples, context=context)
