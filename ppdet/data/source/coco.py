...
class COCODataSet(DetDataset):
    """
    Load dataset with COCO format. 加载coco格式的数据集
    ...
    def parse_dataset(self):
        ...
        assert anno_path.endswith('.json'), \
            'invalid coco annotation file: ' + anno_path #coco格式的标注文件必须是json文件
        from pycocotools.coco import COCO
        coco = COCO(anno_path)
        img_ids = coco.getImgIds() #图像id列表
        img_ids.sort() 
        cat_ids = coco.getCatIds() #类别id列表
        records = []
        empty_records = []
        ct = 0
        ...
        for img_id in img_ids: #对于每张图像的id
            #加载图像信息：文件名、宽、高
            ...
            #构建图像基础记录
            coco_rec = {
                'im_file': im_path, #图像文件路径
                'im_id': np.array([img_id]), #图像id
                'h': im_h, #高
                'w': im_w, #宽
            ...
            if not self.load_image_only:
                ...
                ins_anno_ids = coco.getAnnIds(
                    imgIds=[img_id], iscrowd=None if self.load_crowd else False) #与图像相关的标注id
                instances = coco.loadAnns(ins_anno_ids) #根据标注id加载标注信息
                ...
                for inst in instances:
                    # check gt bbox
                    if inst.get('ignore', False): #忽略ignore=True的标注
                        continue
                    if 'bbox' not in inst.keys(): #跳过没有bbox字段/bbox为空的标注
                        continue
                    else:
                        if not any(np.array(inst['bbox'])):
                            continue

                    #转换标注框格式：将[x1, y1, w, h] 转换为 [x1, y1, x2, y2]
                    x1, y1, box_w, box_h = inst['bbox']
                    x2 = x1 + box_w
                    y2 = y1 + box_h
                    #确保面积大于0且边长大于极小值eps，将合法的框存入bboxes列表
                    eps = 1e-5
