...
class Trainer(object):
    def __init__(self, cfg, mode='train'):
        ...
        if cfg.architecture == 'YOLOX':  #特殊模型YOLOX的参数调整
            for k, m in self.model.named_sublayers():
                if isinstance(m, nn.BatchNorm2D):
                    m._epsilon = 1e-3  # for amp(fp16)
                    m._momentum = 0.97  # 0.03 in pytorch

        #加载归一化参数：从 TestReader 的 sample_transforms 字段中获取
        ...
        if self.mode == 'eval':
            if cfg.architecture == 'FairMOT':
                ...
                self._eval_batch_sampler = paddle.io.BatchSampler( #批次采样器
    ...
    def train(self, validate=False): 
        ...
        Init_mark = False
        #validate：是否在训练过程中评估，若是则创建EvalDataset
        if validate: 
    ...
    def predict(self,
                images,
                images_modal2=None,  # 新增：第二模态图片（可选）
                draw_threshold=0.5,
                output_dir='infer_output/llvip',
                save_results=False,
                visualize=True):
        """
        通用推理方法，支持单模态和双模态
        Args:
            images: 主模态图片路径列表
            images_modal2: 第二模态图片路径列表（可选，用于双模态）
        """
        # 判断是单模态还是双模态
        if images_modal2 is not None:
            return self.predict_dual_modal(images, images_modal2, draw_threshold, 
                                         output_dir, save_results, visualize)
        
        # 单模态推理（保持原有逻辑）
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        self.dataset.set_images(images) #将图片数据传入数据集
        loader = create('TestReader')(self.dataset, 0)
        ...
        if visualize:
            for outs in results:
                ...
                for i, im_id in enumerate(outs['im_id']):
                    ...
                    start = end
        # 这里可以加一行打印，检查每个检测框的类别索引和类别名
        if 'bbox' in outs:
            # 假设outs['bbox']的shape为[N, 6]，其中第1列为类别索引
            for box in outs['bbox']:
                class_id = int(box[0])
                print(f"预测类别索引: {class_id}, 类别名: {catid2name.get(class_id, '未知')}")

        return results

    def predict_dual_modal(self,
                      images_modal1,
                      images_modal2,
                      draw_threshold=0.5,
                      output_dir='output',
        ...
        # 设置双模态数据
        self._setup_dual_modal_dataset(images_modal1, images_modal2)
        ...
        # 设置指标（如果需要保存结果）
        if save_results:
            metrics = self._setup_metrics_for_loader(output_dir, imid2path)
        else:
            metrics = []
        ...
        # 可视化（选择哪个模态进行可视化）
        if visualize:
            self._visualize_dual_modal_results(results, imid2path, clsid2catid, 
                                             catid2name, draw_threshold, output_dir)

        return results

    def _setup_dual_modal_dataset(self, images_modal1, images_modal2):
        """设置双模态数据集"""
        assert len(images_modal1) == len(images_modal2), "两个模态的图片数量必须相同"
        
        if hasattr(self.dataset, 'set_dual_images'):
            self.dataset.set_dual_images(images_modal1, images_modal2)
        else:
            # 手动设置双模态数据
            dual_images = []
            for i, (img1, img2) in enumerate(zip(images_modal1, images_modal2)):
                dual_images.append({
                    'im_file': img1,
                    'im_file_1': img2,  # 第二模态
                    'im_id': i
                })
            self.dataset.roidbs = dual_images
            self.dataset._imid2path = {i: img1 for i, img1 in enumerate(images_modal1)}
            self.dataset._imid2path_modal2 = {i: img2 for i, img2 in enumerate(images_modal2)}  # 新增：模态2路径映射

    def _setup_metrics_for_loader(self, output_dir, imid2path):
        """为加载器设置指标（用于推理时保存结果）"""
        # mem
        metrics = copy.deepcopy(self._metrics)
        mode = self.mode
    ...
    def _visualize_dual_modal_results(self, results, imid2path, clsid2catid, 
                                catid2name, draw_threshold, output_dir):
        """双模态结果可视化 - 支持两个模态"""
        for outs in results:
            batch_res = get_infer_results(outs, clsid2catid)
            ...
            for i, im_id in enumerate(outs['im_id']):
                ...
                # 方案1：分别保存两个模态的可视化结果
                self._save_both_modals_separately(
                    im_id, bbox_res, mask_res, segm_res, keypoint_res, pose3d_res,
                    catid2name, draw_threshold, output_dir)
                
                # 方案2：拼接两个模态到一张图片
                # self._save_concatenated_modals(
                #     im_id, bbox_res, mask_res, segm_res, keypoint_res, pose3d_res,
                #     catid2name, draw_threshold, output_dir)
                
                start = end

    def _save_both_modals_separately(self, im_id, bbox_res, mask_res, segm_res, 
                                   keypoint_res, pose3d_res, catid2name, 
                                   draw_threshold, output_dir):
        """分别保存两个模态的可视化结果"""
        # 获取两个模态的图片路径
        modal1_path = self.dataset._imid2path[int(im_id)]  # 模态1路径
        modal2_path = self.dataset._imid2path_modal2[int(im_id)]  # 模态2路径（需要在数据集中添加）
        
        # 模态1可视化
        image1 = Image.open(modal1_path).convert('RGB')
        image1 = ImageOps.exif_transpose(image1)
        image1_vis = visualize_results(
            image1, bbox_res, mask_res, segm_res, keypoint_res,
            pose3d_res, int(im_id), catid2name, draw_threshold)
        
        save_name1 = self._get_save_image_name(output_dir, modal1_path, suffix='_visible')
        image1_vis.save(save_name1, quality=95)
        logger.info("Modal1 detection results saved in {}".format(save_name1))
        
        # 模态2可视化
        image2 = Image.open(modal2_path).convert('RGB')
        image2 = ImageOps.exif_transpose(image2)
        image2_vis = visualize_results(
            image2, bbox_res, mask_res, segm_res, keypoint_res,
            pose3d_res, int(im_id), catid2name, draw_threshold)
        
        save_name2 = self._get_save_image_name(output_dir, modal2_path, suffix='_thermal')
        image2_vis.save(save_name2, quality=95)
        logger.info("Modal2 detection results saved in {}".format(save_name2))

    def _save_concatenated_modals(self, im_id, bbox_res, mask_res, segm_res, 
                                keypoint_res, pose3d_res, catid2name, 
                                draw_threshold, output_dir):
        """拼接两个模态到一张图片"""
        import numpy as np
        ...
        # 获取两个模态的图片
        modal1_path = self.dataset._imid2path[int(im_id)]
        modal2_path = self.dataset._imid2path_modal2[int(im_id)]
        ...
        # 水平拼接两张图片
        img1_array = np.array(image1_vis)
        img2_array = np.array(image2_vis)
        ...
        if h1 != h2:
            ...
            target_h = min(h1, h2)
            if h1 > target_h:
                img1_array = img1_array[:target_h, :, :]
            if h2 > target_h:
                img2_array = img2_array[:target_h, :, :]
        
        # 水平拼接
        concatenated = np.concatenate([img1_array, img2_array], axis=1)
        concat_image = Image.fromarray(concatenated)
        
        save_name = self._get_save_image_name(output_dir, modal1_path, suffix='_dual_modal')
        concat_image.save(save_name, quality=95)
        logger.info("Dual modal detection results saved in {}".format(save_name))

    def _get_save_image_name(self, output_dir, image_path, suffix=''):
        """生成保存图片的名称"""
        image_name = os.path.split(image_path)[-1]
        name, ext = os.path.splitext(image_name)
        return os.path.join(output_dir, "{}{}{}".format(name, suffix, ext))
    ...
    def _get_infer_cfg_and_input_spec(self,
        ...
        '''获取静态模型和输入规格：
            输入：
                save_dir保存路径；prune_input是否裁剪输入规格；kl_quant是否启用量化
            返回：
                静态模型路径；输入规格
        '''
        ...
        scale_factor = [None, 2]
        #确定testReader
        if self.cfg.architecture in MOT_ARCH: #多目标跟踪模型
            test_reader_name = 'TestMOTReader'
        else:
            test_reader_name = 'TestReader'
        
        #从配置文件中读取图像形状：若未指定则默认形状[任意批次, 3通道, 任意高宽]
        ...
        # if self.cfg.PPHGNetV2X6deploy is not None: #某些模型（如 PPHGNetV2X6）需要多通道输入。
        #     image_shape = [None, 6, -1, -1]
        ...
        input_spec = [{
            ...
            "image_1": InputSpec(
            shape=image_shape, name='image_1'),  # 添加第二个图像输入
            "im_shape": InputSpec(
    ...
    def export(self, output_dir='output_inference'):
        '''将训练好的模型从动态图转为静态图，并导出为推理格式：
            output_dir指定导出目录'''
        #删除辅助结构（如辅助颈部和辅助头部）：训练时需要，推理时不需要
        ...
        # 检查是否设置了 full_graph 参数
        full_graph = self.cfg.get('full_graph', True)  # 默认为 True
